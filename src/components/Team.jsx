...
export const Team = ({ logo, name, points, group }) => {
  return (
    <div className=" text-center">
      <button
        disabled
        className="skew-x-[-5deg]  cursor-default bg-gray-700 border-2 rounded-lg w-2/3 h-44 border-black text-white"
      >
        <div
          className={`before:w-6 before:rounded before:content-[''] before:h-40 before:-top-[17px]  before:absolute before:left-0 before:border-t-[18px]  ${groupColor[group]} w-full h-full border-2  border-black rounded-lg border-b-[18px] border-b-slate-800 border-t-[18px] border-t-slate-600 relative border-x-transparent`}
        >
          <div className="w-full h-full flex flex-row gap-4 px-6 justify-center items-center text-white">
            <div>
              <img src={logo} className="w-36 h-36 object-cover" />
            </div>
